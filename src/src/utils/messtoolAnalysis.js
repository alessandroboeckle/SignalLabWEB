...
// Sliding-window RMS: RMS computed per time window, stepped forward by
// windowSec * (1 - overlapPct/100) each time — an "RMS over time" trend
// instead of one single scalar for the whole signal. Common for spotting
// load/vibration events that a single overall RMS averages away.
//
// Each returned point's time is the window's CENTER (not its start), so
// plotting {t, rms} lines the trend up correctly against the original
// signal rather than shifting it earlier by half a window.
//
...
export function rollingRms(y, t, windowSec, overlapPct = 0) {
  ...
  for (let winStart = t0; winStart + windowSec <= tEnd + 1e-9; winStart += stepSec) {
    ...
    while (i < t.length && t[i] <= winEnd) {
      const v = y[i];
      if (v != null && Number.isFinite(v)) { s += v * v; n++; }
      i++;
    }
    if (n > 0) {
